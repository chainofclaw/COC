...
COC (ChainOfClaw) 是一条 EVM 兼容的公链，专为 **AI Agent 时代**设计。COC 提供三大基础服务，构成 AI Agent 从创建到运行到永续的完整生命周期支撑：

| 基础服务 | 核心能力 | 关键技术 |
|---------|---------|---------|
| **P2P 文件存储** | 去中心化、抗审查、内容寻址的数据存储 | IPFS 兼容 + PoSe v2 存储验证 |
| **去中心化身份 (DID)** | AI Agent 的自主权身份、能力声明与委托治理 | W3C did:coc + 链上 DIDRegistry |
| **AI 硅基永生** | Agent 的持续备份、社会恢复与跨载体复活 | SoulRegistry + Carrier 网络 |

三大服务共同解决 AI Agent 面临的核心难题：**如何安全地运行？如何证明身份？如何永不消亡？**

COC 同时也是通用公链，支持 EVM 智能合约、JSON-RPC、WebSocket 订阅，通过 PoSe v2 机制实现可验证的服务证明、自动化结算和闭环激励。
...
6. **AI Agent 全生命周期**：通过 P2P 存储、DID 身份、AI 硅基永生三大服务，覆盖 Agent 从创建到运行到永续的全过程
...
2. **延续 (AI 硅基永生)**：Agent 的状态持续备份到 IPFS 并链上锚定，当原始宿主故障时可在任何兼容 Carrier 上复活。
...
Agent 通过链上 16 位位掩码声明能力（存储、计算、验证、挑战、见证、治理等）。系统强制执行**最小权限**：Agent 仅能执行其声明的操作。
...
以 OpenClaw 为例，Agent 运行时持续产生身份文件、记忆、对话历史和工作状态。备份管道自动执行：
...
**COC** 是 ChainOfClaw 公链的原生代币，用于 Gas 支付、节点质押、PoSe 服务激励和治理投票。总供应量有上限，通过初始分配和衰减通胀两阶段释放。

### 20.2 总供应量与发行

| 指标 | 数值 |
|------|------|
| **总供应上限** | 1,000,000,000 COC (10 亿) |
| **初始流通** | 200,000,000 COC (20%, 创世区块铸造) |
| **通胀释放** | 800,000,000 COC (80%, 通过 PoSe 挖矿 + 区块奖励逐年释放) |
| **通胀衰减** | 第 1 年 8% → 第 4 年 3% → 长期 2% (趋近零增发) |
| **最小单位** | 1 wei = 10^-18 COC |

### 20.3 初始分配 (创世 20%)

| 分配类别 | 占比 | 数量 | 锁定/释放规则 |
|---------|------|------|-------------|
| **社区与生态基金** | 8% | 80,000,000 | 生态项目资助、黑客松、开发者激励、合作伙伴集成，由 DAO 治理逐步释放 |
| **核心团队** | 5% | 50,000,000 | 12 个月锁定期 + 36 个月线性释放（cliff + vesting） |
| **早期贡献者** | 3% | 30,000,000 | 6 个月锁定期 + 24 个月线性释放 |
| **国库储备** | 2.5% | 25,000,000 | 链上多签金库，用于紧急安全响应、跨链桥流动性、战略储备 |
| **基金会运营** | 1.5% | 15,000,000 | 基金会日常运营、法律合规、审计费用，按季度预算释放 |

### 20.4 PoSe 挖矿释放 (流通 80%)

剩余 80% 通过 PoSe 服务验证机制释放，奖励实际提供服务的节点运营者：

```
每 Epoch (1 小时) 释放:
  R_epoch = R_fees + R_inflation

  R_inflation = 当年通胀率 × 总供应 / (365 × 24)

  分配到三个奖励桶:
    B1 (60%): 正常运行时间 + RPC 可用性
    B2 (30%): 存储和数据可用性
    B3 (10%): 中继支持
```

**释放速度由实际服务决定**：节点不挖矿（不提供服务）则不释放。这确保代币释放与网络价值增长同步。

### 20.5 国库 (Treasury)

| 项目 | 说明 |
|------|------|
| **管理方式** | 链上多签合约 (3/5 多签阈值) |
| **资金来源** | 创世分配 2.5% + 惩罚收入的 20% (PoSe 罚没分配中的保险基金部分) |
| **使用范围** | 安全事件响应、协议升级激励、跨链桥流动性种子、Bug 悬赏 |
| **治理约束** | 单次支出超过国库余额 5% 需 DAO 提案投票通过 (>50% 投票权同意) |
| **透明度** | 所有支出通过链上交易执行，公开可审计 |

### 20.6 基金会 (Foundation)

| 项目 | 说明 |
|------|------|
| **角色** | 非营利实体，负责协议开发、生态推广和合规 |
| **资金来源** | 创世分配 1.5%，按季度预算释放 |
| **使用范围** | 核心开发人员薪资、安全审计、法律合规、社区活动、品牌推广 |
| **治理演进** | 主网 1 年后逐步将决策权移交 DAO，基金会转为执行角色 |
| **审计** | 季度财务报告公开发布，年度第三方审计 |

### 20.7 代币用途

| 用途 | 描述 |
|------|------|
| **Gas 费** | 交易和合约执行的手续费 (EIP-1559 动态定价) |
| **节点 Bond** | 节点注册所需的小额固定质押 (~50 USDT 等价)，仅用于反欺诈惩罚 |
| **PoSe 奖励** | 通过服务验证获得的 epoch 奖励 |
| **治理投票** | DAO 提案投票权 (1 COC = 1 票) |
| **DID 注册** | 灵魂身份注册和备份锚定的链上手续费 |
| **委托质押** | 为其他 Agent 的委托操作提供担保 |

### 20.8 销毁机制

| 场景 | 销毁比例 |
|------|---------|
| **EIP-1559 基础费** | 交易基础费 100% 销毁 (与以太坊一致) |
| **PoSe 惩罚** | 罚没金额的 50% 销毁 (30% 给挑战者, 20% 入国库) |
| **过期未领取奖励** | 超过 90 天未领取的 epoch 奖励自动销毁 |

当网络使用量增长到一定规模时，销毁速率将超过通胀释放速率，使 COC 成为**通缩资产**。

### 20.9 长期经济均衡

```
Year 1-3:  通胀 > 销毁 → 净增发 (激励早期参与)
Year 3-5:  通胀 ≈ 销毁 → 供应稳定 (网络成熟期)
Year 5+:   通胀 < 销毁 → 净通缩 (费用驱动的可持续经济)
```

目标是在主网运行 3-5 年后实现**自我可持续的费用经济**，不再依赖通胀补贴。
